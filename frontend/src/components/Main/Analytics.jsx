...
function Analytics({ display }) {
  ...
  const onSubmitHandler = async (e) => {
    ...
    setDemandData(prev => response.dateDemand);
  }
  ...
  return (
    <div>
      ...
      <div className={(!demandData.length ? "hidden" : "block")}>
        <LineChart width={1000} height={300} data={demandData}>
          <XAxis dataKey="name" />
          <YAxis />
          <CartesianGrid stroke="#eee" strokeDasharray="5 5" />
          <Line type="monotone" dataKey="demand" stroke="#000000" />
        </LineChart>
      </div>
    </div>
  )
